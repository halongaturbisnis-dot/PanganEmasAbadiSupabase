...
export const customerService = {
  ...
  async getAll(): Promise<ICustomer[]> {
    const sql = `SELECT * FROM customer WHERE is_deleted = FALSE OR is_deleted IS NULL ORDER BY name ASC`;
    try {
  ...
  async getPaginated(
    ...
    if (search) {
      whereClause += ` AND (name LIKE ? OR telepon LIKE ? OR email LIKE ? OR alamat LIKE ? OR company LIKE ? OR bidang_usaha LIKE ?)`;
  ...
  async getById(id: string): Promise<ICustomer | null> {
    const sql = `SELECT * FROM customer WHERE id = ? AND (is_deleted = FALSE OR is_deleted IS NULL) LIMIT 1`;
    try {
  ...
  async delete(id: string): Promise<boolean> {
    const sql = `UPDATE customer SET is_deleted = TRUE WHERE id = ?`;
    try {
  ...
  async getDistinctBidangUsaha(): Promise<{ label: string; value: string }[]> {
    const sql = `SELECT DISTINCT bidang_usaha FROM customer WHERE bidang_usaha IS NOT NULL AND bidang_usaha != '' AND (is_deleted = FALSE OR is_deleted IS NULL) ORDER BY bidang_usaha ASC`;
    try {
      const result = await dbClient.query(sql);
  ...
  async getOrCreateGeneralCustomer(): Promise<ICustomer> {
    try {
      const sqlCheck = `SELECT * FROM customer WHERE name = 'Customer Umum' AND (is_deleted = FALSE OR is_deleted IS NULL) LIMIT 1`;
