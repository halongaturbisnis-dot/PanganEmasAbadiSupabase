...
export const suplierService = {
  ...
  async getAll(): Promise<ISuplier[]> {
    const sql = `SELECT * FROM suplier WHERE is_deleted = FALSE OR is_deleted IS NULL ORDER BY name ASC`;
    try {
  ...
  async getPaginated(
    ...
    if (search) {
      whereClause += ` AND (name LIKE ? OR telepon LIKE ? OR email LIKE ? OR alamat LIKE ? OR bank_name LIKE ?)`;
  ...
  async getById(id: string): Promise<ISuplier | null> {
    const sql = `SELECT * FROM suplier WHERE id = ? AND (is_deleted = FALSE OR is_deleted IS NULL) LIMIT 1`;
    try {
  ...
  async delete(id: string): Promise<boolean> {
    const sql = `UPDATE suplier SET is_deleted = TRUE WHERE id = ?`;
    try {
