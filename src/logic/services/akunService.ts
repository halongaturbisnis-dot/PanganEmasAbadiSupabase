...
export const akunService = {
  ...
  async getAll(): Promise<IAkun[]> {
    const sql = `SELECT * FROM akun WHERE is_deleted = FALSE OR is_deleted IS NULL ORDER BY created_at DESC`;
    try {
  ...
  async getById(id: string): Promise<IAkun | null> {
    ...
    const sql = `SELECT * FROM akun WHERE id = ? AND (is_deleted = FALSE OR is_deleted IS NULL) LIMIT 1`;
    try {
  ...
  async delete(id: string): Promise<boolean> {
    try {
      const sql = `UPDATE akun SET is_deleted = TRUE WHERE id = ?`;
  ...
  async deleteMany(ids: string[]): Promise<boolean> {
    ...
    try {
      ...
      const sqlUpdate = `UPDATE akun SET is_deleted = TRUE WHERE id IN (${placeholders})`;
      await dbClient.query(sqlUpdate, ids);
      return true;
    } catch (error) {
      errorService.handle(error);
      return false;
  ...
  async authenticate(kode_akses: string, password_plain: string): Promise<{ success: boolean; session?: IAkunSession; error?: string }> {
    if (kode_akses === 'spadmin' && password_plain === 'PanmasCdm1') {
      ...
      browserStorage.set('token', '00000000-0000-0000-0000-000000000000');
      return { success: true, session };
    }

    const sql = `SELECT * FROM akun WHERE kode_akses = ? AND (is_deleted = FALSE OR is_deleted IS NULL) LIMIT 1`;
    try {
  ...
  async getDistinctJabatan(): Promise<{ label: string; value: string }[]> {
    const sql = `SELECT DISTINCT jabatan FROM akun WHERE jabatan IS NOT NULL AND jabatan != '' AND (is_deleted = FALSE OR is_deleted IS NULL) ORDER BY jabatan ASC`;
    try {
      const result = await dbClient.query(sql);
      return (result.rows as any[]).map(row => ({
        label: row.jabatan,
  ...
  async getDrivers(): Promise<IAkun[]> {
    const sql = `SELECT * FROM akun WHERE peran = 'User' AND akses_modul LIKE '%Pengiriman%' AND (is_deleted = FALSE OR is_deleted IS NULL) ORDER BY username ASC`;
    try {
      const result = await dbClient.query(sql);
      return (result.rows as any[]).map((row) => ({
        ...row,
  ...
  async isUsernameTaken(username: string, excludeId?: string): Promise<boolean> {
    ...
    let sql = `SELECT COUNT(*) as total FROM akun WHERE username = ? AND (is_deleted = FALSE OR is_deleted IS NULL)`;
  ...
  async isKodeAksesTaken(kode_akses: string, excludeId?: string): Promise<boolean> {
    ...
    let sql = `SELECT COUNT(*) as total FROM akun WHERE kode_akses = ? AND (is_deleted = FALSE OR is_deleted IS NULL)`;
